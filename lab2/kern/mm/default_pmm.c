...
/*
 *根据page_init传递的参数建立一个连续内存空闲块的双向链表
*/
static void
default_init_memmap(struct Page *base, size_t n) {
    ...
    base->property = n;
    SetPageProperty(base);		//清零此页的引用计数
    nr_free += n;			//计算空闲页总数 
    //list_add(&free_list, &(base->page_link));//delete
    list_add_before(&free_list, &(base->page_link));//将空闲页插入到链表
}
...
default_alloc_pages(size_t n) {
    assert(n > 0);
    if (n > nr_free) {
        return NULL;         //避免分配超出范围
    }
    struct Page *page = NULL;
    list_entry_t *le = &free_list;	//在空闲链表中寻找合适大小的页块
    while ((le = list_next(le)) != &free_list) {
        struct Page *p = le2page(le, page_link);//找到了合适大小的页块
        if (p->property >= n) {
            page = p;
            break;
        }
...
///*
    if (page != NULL) {
        if (page->property > n) {
            struct Page *p = page + n;	//分割的页需要重新设置空闲大小
            p->property = page->property - n;
            SetPageProperty(p);		//第一页重置标志位
            list_add_after(&(page->page_link), &(p->page_link));
        }
        list_del(&(page->page_link));
        nr_free -= n;
        ClearPageProperty(page);	//清除free_list中的链接
    }
//*/
    return page;
}
